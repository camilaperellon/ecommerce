...
class User extends Model{
    ...
    public static function checkLogin($inadmin = true){
        ...
        if(!isset($_SESSION[User::SESSION])  //Verifica se se a sessão não existe
            ...
            || !(int)$_SESSION[User::SESSION]["iduser"] > 0 ){ //se o id do usuário não é maior q 0

            //Não está logado
            return false;
        }else{

            if($inadmin === true && (bool)$_SESSION[User::SESSION]['inadmin'] === true){ //verificando se o usuário tem acesso a area administrativa
	    ...
	    public static function getForgot($email, $inadmin = true)
    {
         ...
         else
         {
             ...
             else
             {
                 ...
                 if ($inadmin === true) {
                     $link = "http://127.0.0.1/curso/Ecommerce/index.php/admin/forgot/reset?code=$result";
                 } else {
                     $link = "http://127.0.0.1/curso/Ecommerce/index.php/forgot/reset?code=$result";
                 } 
                 $mailer = new Mailer($data['desemail'], $data['desperson'], "Redefinir senha da Camila Store", "forgot", array(
                     "name"=>$data['desperson'],
                     "link"=>$link
                 ));
